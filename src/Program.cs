    ...
    class Program
    {
        static int Main(string[] args)
        {
            try
            {
                ...
                LaunchEmailPlug(plugArgs.WebSocketUrl, config, plugArgs.BotName, plugArgs.ApiKey);
                return 0;
            }
            catch (Exception e)
            {
        ...
        static void LaunchEmailPlug(
            string serverUrl, Config config, string plugName, string apiKey)
        {
            //Ignore SSL certificates
            ServicePointManager.ServerCertificateValidationCallback +=
                (senderDir, cert, chain, sslPolicyErrors) => true;
        ...
        static void CheckArguments(PlugArguments plugArgs)
        {
            ...
            CheckAgumentIsNotEmpty("name for this bot", plugArgs.BotName, "name", "--name email");
        ...
        static Config ReadConfigFile(string botFilePath)
        {
            try
            {
                ...
                CheckFieldIsNotEmpty("senderEmail", result.SenderEmail);
                CheckFieldIsNotEmpty("senderPassword", result.SenderPassword);
                CheckFieldIsNotEmpty("smtpServer", result.SmtpServer);
                if (result.SmtpPort == 0)
                    throw BuildFieldNotDefinedException("smtpPort");
                return result;
            }
            catch (Exception e)
            {
            ...
            string fielName, string fieldValue, string type, string example)
        {
            if (!string.IsNullOrEmpty(fieldValue))
                return;
            string message = string.Format("emailplug can't start without specifying a {0}.{1}" +
        ...
        static void PrintUsage()
        {
            ...
            Console.WriteLine("\temailplug.exe --server <WEB_SOCKET_URL> --config <JSON_CONFIG_FILE_PATH>");
            Console.WriteLine("\t              --apikey <WEB_SOCKET_CONN_KEY> --name <PLUG_NAME>");
            Console.WriteLine();
            Console.WriteLine("Example:");
            Console.WriteLine("\temailplug.exe --server wss://blackmore:7111/plug --config email-config.conf ");
            Console.WriteLine("\t              --apikey x2fjk28fda --name email");
            Console.WriteLine();
        }
